...
// person behind it. Set the same value on both sides:
...
http.route({
  ...
  handler: httpAction(async (ctx, request) => {
    ...
    // An unset secret must fail closed. Comparing against undefined would
    // otherwise let a caller that sends no header at all straight through.
    if (!secret || request.headers.get("x-poller-secret") !== secret) return unauthorized();
